...
with _tab1:
    ...
    with colA:
        st.write("**Model (.pth)**")
        j_up = st.file_uploader("Upload Johnson  trained module", type=["pth", "pt"], accept_multiple_files=False, key="j_ckpt")
    ...
    if results_j:
        st.subheader("Preview — Johnson")
        st.image(results_j[0][1], use_column_width=True)
        st.markdown("---")
...
with _tab2:
    ...
    with c1:
        ...
        a_up = st.file_uploader("Upload Adain trained module", type=["pth", "pt"], accept_multiple_files=False, key="a_ckpt")
